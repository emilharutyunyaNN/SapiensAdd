...
from .loops import EpochBasedTrainLoop, IterBasedTrainLoop, TestLoop, ValLoop, TrainLSTMLoop
...
class Runner:
    ...
    def __init__(
        ...
        temporal_model: Union[nn.Module, Dict],
        work_dir: str,
        ...
        print("val_loop: ", self._val_dataloader, "  ",self._val_loop, "   ",self._val_evaluator  )
        #return 
        ...
        #print("test dataloader: ", self._test_dataloader)
        self._launcher = launcher
        if self._launcher == 'none':
            self._distributed = False
        else:
            self._distributed = True
        #print("tmp: ", self.cfg.temp)
        #return
        ...
        self.temporal_model = self.build_model(temporal_model)
        #print("fine")
        #return
        # wrap model
        #print("----------", self.cfg.get('model_wrapper_cfg'))
        """local_rank = int(os.environ["LOCAL_RANK"])
        if local_rank in [0,1]:
            self.model = self.wrap_model(
                self.cfg.get('model_wrapper_cfg'), self.model, local_rank)
        elif local_rank ==2:
            
            self.temporal_model = self.wrap_model(
                self.cfg.get('model_wrapper_cfg'), self.temporal_model, local_rank)
        else:
            raise RuntimeError(f"Invalid rank {local_rank}. Only ranks 0, 1, and 2 are valid.")"""
        self.model = self.model.to(f"cuda:{self.cfg.get('device_map')['pose']}")
        self.temporal_model = self.temporal_model.to(f"cuda:{self.cfg.get('device_map')['temporal']}")
        #return
        print("here")
    ...
    def wrap_model(
            ...
            model: nn.Module, device_id: int) -> Union[DistributedDataParallel, nn.Module]:
        ...
        if model_wrapper_cfg is None:
            ...
            if issubclass(
                    ...
                    DistributedDataParallel):
                default_args['device_ids'] = [device_id]
    ...
    def build_dataloader(dataloader: Union[DataLoader, Dict],
        ...
        if isinstance(dataset_cfg, dict):
            ...
            #print("Dataset right after building: ", dataset[:4])
